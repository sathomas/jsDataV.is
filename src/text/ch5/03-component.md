...
``` {.html .numberLines}
<div style='position:relative;height:600px;'><div id='timeline'></div></div>
```
...
To use our own fonts, all we need to do is copy that file and replace `'News Cycle'` and `'Merriweather'` with our own choices. Perhaps we'd prefer Rokkitt and Lato.
...
timelinejs = new VMM.Timeline('timelinejs-1','100%','600px');
timelinejs.init(timelineConfig);
...
timelinejs2 = new VMM.Timeline('timelinejs-2','100%','600px');
